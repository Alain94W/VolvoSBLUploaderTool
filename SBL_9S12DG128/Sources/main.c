/*
    ...
    Change the start address of the program and the RAM usage in the prm file.
    ex : 
     ...
     ...
     ...
    ==> Program will start at 0x1400 in RAM and will use ROM function at RAM_1000 
*/
#include <hidef.h>
#include <string.h>
#include "CANLib_H.h"
...
//#define USED_CAN_INTERFACE 0   // For CEM id 0x40  and DIM id 51
 ...
 if (DataAvailable(CAN_INTERFACE) == 1) {
  ...
  if ((Recpt.RxBuff[0] != CAN_ECU_ID) && (Recpt.RxBuff[0] != ALL_ECU_ID)) continue;
    ...
    if (Recpt.RxBuff[1] == 0xD0) {
     ...
     // A8,C8 : Exit SBL, RESET ECU into NORMAL mode
     ...
     if ((Recpt.RxBuff[1] == 0xF8) || (Recpt.RxBuff[1] == 0xDA)) {
        ...
        if (Type == 1) {
          if ( page>0x00 )
          {
            BufPntr[0]=CAN_ECU_ID;
            BufPntr[1]=Recpt.RxBuff[1]+1;
            BufPntr[2]=0x0B; // Invalid EEPROM ADDR
            BufPntr[3]=0x00;
            BufPntr[4]=0x00;
            BufPntr[5]=0x00;
            BufPntr[6]=0x00;
